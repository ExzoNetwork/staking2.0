...
function BlockStake(props) {
  ...
  const buttonsStyle = activateButtons
    ? styles.styleBtnGreen
    : styles.styleBtnDisabled;
  const stakeClick = () => {
    if (!validatorDetails.stakeDataIsLoaded) return;
    props.onClickStake();
  };
  const requestBtnStyle = activateButtons
    ? styles.styleBtnRed
    : styles.styleBtnRequestDisabled;
  ...
  return (
    <div
      ...
      {isStakedValidator ? (
        <>
          <div style={rowStyle}>
            <Rectangle
              ...
              style={{
                ...leftRectangle,
                backgroundColor: info.app.bgItem || '#1a1f4f',
              }}
        ...
        <>
          <div
            style={{...rowStyle, marginTop: 20, display: 'flex'}}
...
function BlockWithdrawals(props) {
  ...
  const totalWithdRequested = totalWithdrawRequested
    ? totalWithdrawRequested && totalWithdrawRequested.isZero()
      ? '0'
      : formatStakeAmount(totalWithdrawRequested)
    : '...';
  const totalWithdrawRequestedSign = totalWithdrawRequested
    ? totalWithdrawRequested.isZero()
      ? ''
      : ''
    : '';
  return (
      ...
      <div
        ...
        <div
          style={{
            ...
            backgroundColor:
              idx % 2 == 1
                ? info.app.bgItem || '#252847'
                : info.app.bgSecond || '#161A3F',
  ...
  const EmptyList = (props) => {
    ...
    return (
      <div style={styleEmpty} className="tabs-container-style-empty">
        <div
          style={{marginBlock: 10, fontSize: 12}}
    ...
    <Observer>
      {() => {
        ...
        return (
          <>
            <div
              style={{...tableRow, height: 25, alignItems: 'flex-start'}}
              className="tabs-container-tablerow">
              <div style={tabRowTxt} className="tabs-container-tab-row-txt">
                {'#'}
                {lang.epoch1 || 'Epoch'}
              </div>
              <div style={tabRowTxt} className="tabs-container-tab-row-txt">
...
const TabsContainer = (props) => {
  ...
  useEffect(() => {
    const stakingAccounts =
      props.stakingStore?.chosenValidator?.stakingAccounts || [];
    // Switch to 1st tab in case there is no stake accounts left (double check)
